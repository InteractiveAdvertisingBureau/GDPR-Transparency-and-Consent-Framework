...
For any server side call, if using openRTB, the consent payload should be sent accordingly to the openRTB specs.

For any client side call, once the consent payload has been obtained leveraging the CMP, you may validate that it reflects user-intentful consent such as:
- the field `cmpStatus` is `loaded`, and
- the field `eventStatus` is `tcloaded` or `useractioncomplete`.

After validating the TC data payload is suitable for your case, you should pass it in the ad call using the URL-passing macro solution detailed in documentation for the TC String.
